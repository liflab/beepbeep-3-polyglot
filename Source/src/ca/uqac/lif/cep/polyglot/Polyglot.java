...
public class Polyglot
{
  ...
  public static void main(String[] args)
  {
    ...
    else
    {
      ...
      {
        System.err
            .println("ERROR: at least one input file and one specification file must be given");
        System.exit(ERR_ARGS);
      }
      String trace_filename = filenames.get(0);
      try
      {
        reader = new ReadStringStream(
            new BufferedInputStream(new FileInputStream(new File(trace_filename))));
  ...
  public static Processor getProcessor(String filename, String in_format)
      throws InvalidGrammarException, FileNotFoundException, BuildException
  {
  ...
  protected static CliParser setupCli()
  {
    ...
    parser.addArgument(new Argument().withLongName("in-format").withArgument("f")
        .withDescription("Sets input format to f"));
    parser
        .addArgument(new Argument().withLongName("stdin").withDescription("Read input from stdin"));
    parser.addArgument(new Argument().withLongName("quiet").withShortName("q")
        .withDescription("Don't print start banner"));
    return parser;
  }
