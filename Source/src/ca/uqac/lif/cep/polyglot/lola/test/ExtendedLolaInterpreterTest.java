...
public class ExtendedLolaInterpreterTest
{
      ...
      throws ParseException, ConnectorException, InvalidGrammarException, BuildException
  {
    ...
    NamedGroupProcessor gp = (NamedGroupProcessor) my_int.build(Util.convertStreamToString(
        ExtendedLolaInterpreterTest.class.getResourceAsStream("test1.lola")));
      ...
      throws ParseException, ConnectorException, InvalidGrammarException, BuildException
  {
    ...
    ExtendedLolaInterpreter my_int = new ExtendedLolaInterpreter();
    NamedGroupProcessor gp = (NamedGroupProcessor) my_int.build(Util.convertStreamToString(
        ExtendedLolaInterpreterTest.class.getResourceAsStream("test2.lola")));
    Connector.connect(src1, 0, gp, gp.getInputIndex("s1"));
    Pullable p = gp.getPullableOutput("s");
    o = p.pull();
    assertEquals(3f, o);
