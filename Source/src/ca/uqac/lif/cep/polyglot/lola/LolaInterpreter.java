...
public class LolaInterpreter extends ca.uqac.lif.cep.dsl.MultilineGroupProcessorBuilder
{
  ...
  protected LolaInterpreter(boolean b)
  ...
  public String getGrammar()
  {
    return ca.uqac.lif.cep.polyglot.Util
        .convertStreamToString(LolaInterpreter.class.getResourceAsStream("lola-core.bnf"));
  }
   ...
   * 
   * @param name
   *          The name of the LOLA trigger
   * @param t
   *          The {@link Triggerable} object that will be called when the trigger
   *          turns true
   */
  ...
  public synchronized GroupProcessor endOfFileVisit()
  {
    ...
    {
      Fork f = entry.getValue();
      int o_arity = f.getOutputArity();
      f.extendOutputArity(f.getOutputArity() + 1);
      Passthrough pt = new Passthrough();
      Connector.connect(f, o_arity, pt, 0);
